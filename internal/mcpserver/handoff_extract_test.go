...
func TestExtractTicketHints_RequiresTwoMentionsOrURL(t *testing.T) {
	msgs := []*connectors.Message{
		msgUser("hey let's look at TICKET-1362 — needs the Payment Link pill."),
		msgAssistant("ok looking at TICKET-1362"),
		msgUser("also TICKET-1362 has the Track Order recovery requirement"),
		msgUser("there is also PROJ-77 mentioned once which should be ignored"),
		msgUser("see https://linear.app/example/issue/TICKET-1361 for the backend"),
	}
	got := extractTicketHints(msgs)
	sort.Slice(got, func(i, j int) bool { return got[i].Key < got[j].Key })

	want := []TicketHint{
		// TICKET-1361 — appears only inside a URL, qualifies via FromURL.
		{Key: "TICKET-1361", Mentions: 0, FromURL: true},
		// TICKET-1362 — 2 mentions across user turns (assistant doesn't count).
		{Key: "TICKET-1362", Mentions: 2, FromURL: false},
	}
...
func TestExtractTicketHints_AssistantOnlyMentionsDoNotCount(t *testing.T) {
	msgs := []*connectors.Message{
		msgAssistant("TICKET-1362 looks tricky"),
		msgAssistant("TICKET-1362 second mention from assistant"),
	}
...
func TestExtractLinkedURLs_DeduplicatesPreservingOrder(t *testing.T) {
	msgs := []*connectors.Message{
		msgUser("see https://linear.app/example/issue/TICKET-1362"),
		msgUser("and https://github.com/example/foo/pull/42 too"),
		msgUser("re-pasting https://linear.app/example/issue/TICKET-1362"),
	}
	got := extractLinkedURLs(msgs)
	want := []string{
		"https://linear.app/example/issue/TICKET-1362",
		"https://github.com/example/foo/pull/42",
	}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("got %v, want %v", got, want)
	}
