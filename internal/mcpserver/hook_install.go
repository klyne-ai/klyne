...
// stopHookCommandSuffix is the argv klyne is invoked with from
// the Stop hook. Writes a deterministic session-end summary into
// klyne's stop_summaries table.
const stopHookCommandSuffix = "session-end"
...
// InstallStopHook wires `klyne session-end` into Claude Code's Stop
// event hook so the deterministic session-end summary is written at
// the end of every session. Idempotent in the same way as
// InstallAdvisorHook — re-running only rewrites when the entry would
// actually change.
//
// Schema parallels the UserPromptSubmit install path:
//
//	{ "hooks": { "Stop": [ {"matcher":"*","hooks":[{"type":"command","command":"<klyne-path> session-end"}]} ] } }
//
// The Stop hook runs once per session-end. It must never block the
// CLI's exit, so the subcommand sets its own deadline and always
// returns exit 0. See cmd/klyne/session_end.go.
func InstallStopHook(binaryPath string) (*HookInstallReport, error) {
	path, err := claudeSettingsPath()
	if err != nil {
		return nil, err
	}
	...
	updatedEntries, action := mergeStopHookEntry(entries, binaryPath)
	if action == InstallActionAlreadyInstalled {
		return &HookInstallReport{Path: path, Action: action}, nil
	}
	hooks["Stop"] = updatedEntries
	root["hooks"] = hooks
...
// mergeStopHookEntry mirrors mergeAdvisorHookEntry for the Stop
// event: ensure exactly one "klyne session-end" hook entry with
// the canonical binary path, preserving any other Stop hooks the
// user has wired for other tools.
func mergeStopHookEntry(entries []any, binaryPath string) ([]any, InstallAction) {
	want := map[string]any{
		"type":    "command",
		"command": binaryPath + " " + stopHookCommandSuffix,
	}

	totalKlyne, canonicalKlyne := scanKlyneSessionEnd(entries, want)
	if totalKlyne == 1 && canonicalKlyne == 1 {
		return entries, InstallActionAlreadyInstalled
	}
	...
	for i, raw := range updated {
		...
		for _, h := range inner {
			...
			if isKlyneSessionEndCommand(hookObj) {
				continue
			}
			filtered = append(filtered, hookObj)
		}
...
// scanKlyneSessionEnd walks "*" / unmatched Stop entries and counts
// how many klyne session-end hooks they carry plus how many match
// the canonical want shape.
func scanKlyneSessionEnd(entries []any, want map[string]any) (total, canonical int) {
	for _, raw := range entries {
		entry, ok := raw.(map[string]any)
		if !ok {
			continue
...
// isKlyneSessionEndCommand reports whether the given hook object is
// a klyne session-end command. Same shape check as the advise hook,
// just looking for the session-end suffix instead.
func isKlyneSessionEndCommand(h map[string]any) bool {
	if t, _ := h["type"].(string); t != "command" {
		return false
	}
	cmd, _ := h["command"].(string)
	...
	if hasSuffix(cmd, " "+stopHookCommandSuffix) {
		return containsKlyneToken(cmd)
	}
	return false
}
